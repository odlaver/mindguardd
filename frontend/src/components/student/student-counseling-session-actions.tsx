...
export function StudentCounselingSessionActions({
  ...
  return (
    <SectionCard title="Aksi Siswa" className="p-5 sm:p-6">
      <div className="grid gap-4">
        {status === "Menunggu Konfirmasi" ? (
          <>
            <p className="text-sm leading-7 text-ink-soft">
              Konfirmasi dulu jadwal dari guru BK. Kamu bisa menambahkan catatan
              singkat sebelum menyetujui sesi.
            </p>
          ...
          <>
            <div className="rounded-[24px] border border-stroke bg-[#f7f8f4] p-4 text-sm leading-7 text-ink-soft">
              {studentConfirmationNote?.trim()
                ? studentConfirmationNote
                : "Jadwal sudah dikonfirmasi. Tambahkan catatan setelah sesi selesai."}
            </div>
            <label className="grid gap-2">
